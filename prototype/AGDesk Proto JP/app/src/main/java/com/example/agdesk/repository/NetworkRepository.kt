...
class NetworkRepository @Inject constructor(private val assetRepository: AssetRepository,
    ...
    suspend fun syncFromServer() {
        ...
        try {
            ...
            dbSyncDAO.insertSyncInfo(DbSync(1, System.currentTimeMillis().toString()))
    ...
    suspend fun syncToServer() {
        ...
        try {
            val response: SyncWrite = httpClient.post("https://httpbin.org/#/HTTP_Methods/post_post") {
                contentType(ContentType.Application.Json)
                setBody(syncWrite)
            }.body()
